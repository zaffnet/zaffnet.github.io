...
![How the multimodal reranker is actually wired](/assets/img/qareen-reranker.svg)
...
The diagram above shows the exact loop we shipped: normalize both modalities, blend with a 0.6 text weight, and run an MMR-style reranker to keep the few-shot set diverse instead of a stack of near-duplicates. That replay log at the end was useful when a new embedding model shifted score scales and we needed to prove where the drift started.
